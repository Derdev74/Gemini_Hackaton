...
async def chat_endpoint(request: Request, chat_req: ChatRequest):
    """
    Main chat endpoint handling the flow directly since we don't have a manager.
    """
    try:
        # 1. Directly call your chatbot_agent (the orchestrator)
        # Make sure chatbot_agent is initialized at the top of this file!
        ai_result = await chatbot_agent.async_process(
            query=chat_req.message, 
            context={"profile": chat_req.preferences}
        )

        # 2. Return the formatted response to your React frontend
        # Inside your chat_endpoint return
        return {
            "agent": "optimizer" if ai_result.get("type") == "optimization_update" else "chatbot",
            "status": "success",
            "message": ai_result.get("text") or "I'm processing your request! One moment.",
            # If data is missing, send an empty list so the Frontend doesn't crash
            "itinerary": ai_result.get("data") if ai_result.get("data") else None,
            "profile": chat_req.preferences
        }
...
async def chat(request: Request):
    ...
    # This now handles the reassurance AND the optimization in one call
    result = await chatbot.async_process(body['query'], body['context'])
    return result
